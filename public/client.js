...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function startMultiPlayer() {
        ...
        // Obtén tu número de jugador del server
    ...
    function checkForWins() {
        ...
        if (cpuDestroyerCount === 2) {
            infoDisplay.innerHTML = `${enemy} hundió tu destructor`
            cpuDestroyerCount = 10
        }
        if (cpuSubmarineCount === 3) {
            infoDisplay.innerHTML = `${enemy} hundió tu submarino`
            cpuSubmarineCount = 10
        }
        if (cpuCruiserCount === 3) {
            infoDisplay.innerHTML = `${enemy} hundió tu bote`
            cpuCruiserCount = 10
        }
        if (cpuBattleshipCount === 4) {
            infoDisplay.innerHTML = `${enemy} hundió tu barco`
            cpuBattleshipCount = 10
        }
        if (cpuCarrierCount === 5) {
            infoDisplay.innerHTML = `${enemy} hundió tu carguero`
            cpuCarrierCount = 10
        }
